  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-slate-900">
      <View className="flex-row items-center justify-between px-4 py-4 border-b border-slate-100 dark:border-slate-800">
        ...
        <TouchableOpacity
          onPress={() => {/* Possible Share or Download function */}}
          className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-full items-center justify-center"
        >
      ...
      <ScrollView className="flex-1 px-4 py-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-8 bg-indigo-600 rounded-2xl p-6 shadow-lg">
          <View className="flex-row justify-between items-start mb-4">
            <View>
              <Text className="text-indigo-100 text-xs font-outfit-bold mb-1">ORDER ID</Text>
          ...
          <View className="flex-row justify-between items-end">
            <View>
              <Text className="text-indigo-100 text-xs font-outfit-bold mb-1">DATE</Text>
              <Text className="text-white font-outfit-medium">
                {formatDate(order.orderDate || order.createdAt)}
              </Text>
            </View>
            <View className="items-end">
              <Text className="text-indigo-100 text-xs font-outfit-bold mb-1">TOTAL AMOUNT</Text>
              <Text className="text-white text-2xl font-outfit-black">৳{order.amount.toFixed(2)}</Text>
            </View>
          </View>
        </View>
        ...
        {/* Items List */}
        <View className="mb-8">
          <Text className="text-lg font-outfit-bold text-slate-900 dark:text-white mb-4">Items Purchased</Text>
          {order.items.map((item: any, index: number) => (
            <View key={index} className="flex-row items-center mb-4 pb-4 border-b border-slate-50 dark:border-slate-800">
              <Image
                source={{ uri: item.image || 'https://via.placeholder.com/150' }}
                className="w-16 h-16 rounded-lg bg-slate-100"
              />
              <View className="flex-1 ml-4">
                <Text className="text-base font-outfit-bold text-slate-900 dark:text-white" numberOfLines={1}>
                  {item.name}
                </Text>
        ...
        <View className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 mb-8">
          <View className="flex-row justify-between mb-2">
            <Text className="text-slate-600 dark:text-slate-400 font-outfit-medium">Subtotal</Text>
            <Text className="text-slate-900 dark:text-white font-outfit-bold">৳{order.amount.toFixed(2)}</Text>
          </View>
          <View className="flex-row justify-between mb-2">
            <Text className="text-slate-600 dark:text-slate-400 font-outfit-medium">Shipping</Text>
            <Text className="text-green-600 font-outfit-bold">FREE</Text>
          </View>
          <View className="flex-row justify-between mb-2">
            <Text className="text-slate-600 dark:text-slate-400 font-outfit-medium">Tax</Text>
            <Text className="text-slate-900 dark:text-white font-outfit-bold">৳0.00</Text>
          </View>
          <View className="h-px bg-slate-200 dark:bg-slate-700 my-2" />
          <View className="flex-row justify-between items-center mt-1">
            <Text className="text-lg font-outfit-black text-slate-900 dark:text-white">Total</Text>
        ...
        <View className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-4 mb-8">
          <View className="flex-row items-center mb-3">
            ...
            <Text className="text-lg font-outfit-bold text-slate-900 dark:text-white ml-2">Shipping Address</Text>
          </View>
        ...
        >
          ...
          <Text className="text-white text-lg font-outfit-bold ml-2">Reorder Items</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
